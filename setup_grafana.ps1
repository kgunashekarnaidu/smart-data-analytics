...
$process = Start-Process -FilePath "msiexec.exe" -ArgumentList "/i `"$installerPath`" /quiet /qn" -Wait -PassThru

if ($process.ExitCode -eq 0) {
    Write-Host "[OK] Grafana installed." -ForegroundColor Green
}
elseif ($process.ExitCode -eq 1638) {
    Write-Host "[OK] Grafana already installed (existing version detected)." -ForegroundColor Green
}
else {
    Write-Host "[FAIL] Installation failed (exit code: $($process.ExitCode))." -ForegroundColor Red
    exit 1
}
...
Write-Host "[OK] custom.ini created at: $customIni" -ForegroundColor Green
...
if (-Not (Test-Path $dsDest)) {
    New-Item -ItemType Directory -Path $dsDest -Force | Out-Null
}
...
if (-Not (Test-Path $dbProvDest)) {
    New-Item -ItemType Directory -Path $dbProvDest -Force | Out-Null
}
...
Write-Host "[OK] All provisioning files in place." -ForegroundColor Green
...
try {
    ...
    if ($svc) {
        if ($svc.Status -eq "Running") {
            ...
            Write-Host "[OK] Grafana service restarted." -ForegroundColor Green
        }
        else {
            Start-Service -Name "Grafana"
            Write-Host "[OK] Grafana service started." -ForegroundColor Green
        }
    }
    else {
        ...
        Write-Host "[OK] Grafana server started manually." -ForegroundColor Green
    }
}
catch {
